...
async def create_user(name: str = Form(...),
    ...
    if file.content_type != "application/pdf":
        return HTTPException(status_code=400, detail="O arquivo deve ser um PDF")
...
def get_users(skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    ...
    if not db_users:
        return HTTPException(status_code=status.HTTP_204_NO_CONTENT, detail="No users found.")
    return db_users
